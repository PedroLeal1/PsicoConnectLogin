...
export default function ChatBotPage() {
  ...
  const sendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: Date.now() + 1,
        text:
          "Erro: Não foi possível conectar ao bot. Verifique se o servidor (porta 8000) está a correr e se o CORS está configurado.",
        sender: "bot",
        timestamp: formatTimestamp(),
      };
      setIsLoading(false);
  ...
  return (
    <main className="chat-main-wrapper">
      ...
      <style>{`
        .message.bot a { color: #1C4BB3; text-decoration: underline; }
        .message.bot a:hover { color: #528CFF; }
      `}</style>
    </main>
  );
}
